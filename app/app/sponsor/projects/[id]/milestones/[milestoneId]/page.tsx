...
export default function MilestoneDetailPage({ params }: { params: { id: string; milestoneId: string } }) {
  ...
  const milestone = milestones.find((item) => item.id === params.milestoneId) ?? milestones[0];

  return (
    <div className="space-y-6">
      <SponsorHeader eyebrow="Milestone review" title={`Review ${milestone.title}`} body="Milestone approval and dispute actions now trigger real Sponsor-side changes: milestone status updates, project status movement, escrow changes, alert creation, and activity log entries." badges={[{ label: milestone.state, tone: milestone.state === "disputed" ? "red" : milestone.state === "paid" ? "green" : "gold" }, { label: project.escrowStatus.replace(/_/g, " "), tone: "teal" }]} actions={[{ label: "Project workspace", href: `/app/sponsor/projects/${params.id}`, tone: "ghost" }]} />
