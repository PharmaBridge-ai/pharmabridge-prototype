...
export default function AlertDetailPage({ params }: { params: { alertId: string } }) {
  ...
  const alert = alerts.find((item) => item.id === params.alertId) ?? alerts[0];
  ...
  return (
    <div className="space-y-6">
      <SponsorHeader
        ...
        title={alert.title}
        body="Alert detail keeps the signal, impacted entity, and recommended next step visible so sponsor teams can act quickly."
        badges={[{ label: alert.status.replace(/_/g, " "), tone: alert.status === "critical" ? "red" : alert.status === "high_risk" ? "gold" : "teal" }]}
        actions={[{ label: "Back to intelligence", href: "/app/sponsor/intelligence", tone: "ghost" }]}
      />
      <SponsorCards title="Alert summary" subtitle="Action-oriented context for sponsor review." items={[
        { title: "Summary", body: alert.body, meta: "Context", tone: "slate" },
        { title: "Impacted entity", body: alert.linkedVendorId ?? alert.linkedProgrammeId ?? "Linked workflow item", meta: "Related object", tone: "gold" },
        { title: "Recommended next step", body: "Open the related vendor, programme, or project and review the current sourcing or execution state.", meta: "Suggested action", tone: "teal" },
      ]} />
