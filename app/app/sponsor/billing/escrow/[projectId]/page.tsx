...
export default function EscrowDetailPage({ params }: { params: { projectId: string } }) {
  ...
  return (
    <div className="space-y-6">
      <SponsorHeader
        eyebrow="Escrow"
        title={`Escrow detail for ${params.projectId.replace(/-/g, " ")}`}
        ...
        actions={[{ label: "Return to project", href: `/app/sponsor/projects/${params.projectId}`, tone: "ghost" }]}
      />
      <SponsorMetricGrid metrics={[
        ["Contract value", "$2.14M", "Awarded project value"],
        ["Minimum escrow required", "$620K", "Funding threshold before activation"],
        ["Current balance", `$${project.escrowBalance.toLocaleString()}`, project.escrowStatus.replace(/_/g, " ")],
        ["Funding status", project.status.replace(/_/g, " "), "Project state updates when funded"],
      ]} />
