...
export default function RfpComparisonPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  const { proposals, generateComparison, comparisonRun } = useSponsorDemo();
  const scoped = proposals.filter((proposal) => proposal.rfpId === id);

  return (
    <div className="space-y-6">
      <SponsorHeader eyebrow="AI comparison report" title={`Proposal comparison for ${id}`} body="This report now uses a staged AI generation sequence. The sponsor sees sections populate progressively as technical, commercial, timeline, and risk signals are normalized into a weighted recommendation." badges={[{ label: "AI normalized", tone: "teal" }, { label: "Comparison ready", tone: "gold" }]} actions={[{ label: "Award board", href: `/app/sponsor/rfps/${id}/award`, tone: "primary" }]} />
      <SponsorAiPanel active={comparisonRun.active && comparisonRun.rfpId === id} title="Comparison generation" progress={comparisonRun.active && comparisonRun.rfpId === id ? comparisonRun.progress : 100} stage={comparisonRun.active && comparisonRun.rfpId === id ? comparisonRun.stage : "Recommendation ready"} summary="Generate a weighted sponsor-facing comparison across technical fit, commercials, timeline, and quality confidence.">{comparisonRun.sectionsReady.length ? <div className="text-sm text-slate-600">Sections: {comparisonRun.sectionsReady.join(" • ")}</div> : null}</SponsorAiPanel>
      ...
      <button onClick={() => void generateComparison(id)} className="rounded-full border border-indigo-200 bg-indigo-600 px-5 py-3 text-sm text-white">Refresh comparison</button>
